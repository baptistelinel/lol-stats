...
class TestMatchRequester(unittest.TestCase):
	...
	def test_get_by_id(self):
		self._requests_adapter.http_get.return_value = {'json': {'match_id': 'match_id_123'}}
		response = self._match_requester.get_details('match_id_123')
		self._requests_adapter.http_get.assert_called_once_with('match/v4/matches/match_id_123')
		self.assertEqual(response, {'match_id': 'match_id_123'})
